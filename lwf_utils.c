/*
 * lwf - Wireless Information Library - Shared utility routines
 *
...
static int lwf_ioctl_socket(void)
{
	/* Prepare socket */
	if (ioctl_socket == -1)
	{
...
int lwf_ifup(const char *ifname)
{
	...
	if (lwf_ioctl(SIOCGIFFLAGS, &ifr))
		return 0;

	ifr.ifr_flags |= (IFF_UP | IFF_RUNNING);

	return !lwf_ioctl(SIOCSIFFLAGS, &ifr);
}
...
int lwf_ifmac(const char *ifname)
{
	...
	if (lwf_ioctl(SIOCGIFHWADDR, &ifr))
		return 0;
	...
	return !lwf_ioctl(SIOCSIFHWADDR, &ifr);
}
...
struct lwf_hardware_entry * lwf_hardware(struct lwf_hardware_id *id)
{
	FILE *db;
	char buf[256] = { 0 };
	static struct lwf_hardware_entry e;
	struct lwf_hardware_entry *rv = NULL;

	if (!(db = fopen(LWF_HARDWARE_FILE, "r")))
		return NULL;
					  ...
					  uint8_t defcipher, uint8_t defauth)
{
	...
	if (!memcmp(data, ms_oui, 3) || !memcmp(data, ieee80211_oui, 3))
	{
		switch (data[3])
		{
			case 1: c->group_ciphers |= LWF_CIPHER_WEP40;  break;
			case 2: c->group_ciphers |= LWF_CIPHER_TKIP;   break;
			case 4: c->group_ciphers |= LWF_CIPHER_CCMP;   break;
			case 5: c->group_ciphers |= LWF_CIPHER_WEP104; break;
	...
	for (i = 0; i < count; i++)
	{
			...
			!memcmp(data + 2 + (i * 4), ieee80211_oui, 3))
		{
			switch (data[2 + (i * 4) + 3])
			{
				case 1: c->pair_ciphers |= LWF_CIPHER_WEP40;  break;
				case 2: c->pair_ciphers |= LWF_CIPHER_TKIP;   break;
				case 4: c->pair_ciphers |= LWF_CIPHER_CCMP;   break;
				case 5: c->pair_ciphers |= LWF_CIPHER_WEP104; break;
				case 6:  /* AES-128-CMAC */ break;
				default: /* proprietary */  break;
			}
		}
	...
	for (i = 0; i < count; i++)
	{
			...
			!memcmp(data + 2 + (i * 4), ieee80211_oui, 3))
		{
			switch (data[2 + (i * 4) + 3])
			{
				case 1: c->auth_suites |= LWF_KMGMT_8021x; break;
				case 2: c->auth_suites |= LWF_KMGMT_PSK;   break;
